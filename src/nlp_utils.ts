...
const wer = (a: string, b: string) => {
    const replaced_a = a.split(/\s+/g).filter(_ => _ != "");
    const replaced_b = b.split(/\s+/g).filter(_ => _ != "");
...
const align_string = (ref: string, hyp: string, _editops: any[][], slice_view = 50) => {
    const ref_array: string[] = ref.replace(/\s/g, ``).match(/./g) || [];
    const hyp_array: string[] = hyp.replace(/\s/g, ``).match(/./g) || [];
    const ref_space_indices: number[] = Array.from(ref.replace(/\s+/g, " ").matchAll(/\s/g)).map(
        el => el.index!
    );
